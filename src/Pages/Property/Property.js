...
function Property(props) {
  ...
  return (
    <>
        ...
        {propertyImage.length > 0 ? (
          <>
            <img
              ...
              src={`https://cdn.repliers.io/${propertyImage[0]}`}
              alt="property images"
            />
            <div className="heroproperty__container">
              ...
              <LightBoxCarousel imagesArr={propertyImage} />
            </div>
          </>
        ) : (
          <>
      ...
      />
      {calculatorIsOpen ? <MortgageCalculator price={propertyPrice} /> : <></>}
      <PropertyDetail
        ...
        built={
          propertyDetail.yearBuilt === null
            ? "N/A"
            : `${propertyDetail.yearBuilt}`
        }
        mls={propertyNumber}
        broker={
          propertyBrokerage.brokerageName === null ||
          propertyBrokerage.brokerageName === ""
            ? "N/A"
            : `${propertyBrokerage.brokerageName}`
        }
        basement={"?????"}
        ...
        tax={
          propertyTax.annualAmount === null
            ? "N/A"
            : `${propertyTax.annualAmount}`
        }
        taxyear={
          propertyTax.assessmentYear === null
            ? "N/A"
            : `${propertyTax.assessmentYear}`
        }
        features={"?????"}
        amenties={"?????"}
        cityCoord={cityCoord}
      ...
      />
      <RequestForm address={propertyAddress.streetName} />
    </>
  );
}
